    ...
    if($confirm === 'yes') {
        try {
            if($action === 'suspend') {
                ...
                $update_stmt = $conn->prepare("UPDATE users SET status = 'suspended', suspension_reason = ?, suspension_until = ?, updated_at = NOW() WHERE id = ?");
                $result = $update_stmt->execute([$reason, $suspension_until, $id]);
                
                if($result) {
                    $success_message = "Student has been suspended successfully!";
                    $action_completed = true;
                } else {
                    $error_message = "Failed to suspend student. Please try again.";
                }
            } else {
                // Unsuspend student
                $update_stmt = $conn->prepare("UPDATE users SET status = 'active', suspension_reason = NULL, suspension_until = NULL, updated_at = NOW() WHERE id = ?");
                $result = $update_stmt->execute([$id]);
                
                if($result) {
                    $success_message = "Student has been unsuspended successfully!";
                    $action_completed = true;
                } else {
                    $error_message = "Failed to unsuspend student. Please try again.";
                }
            }
        } catch(PDOException $e) {
...
if($action === 'unsuspend') {
    $page_title = "Unsuspend Student";
    $action_text = "Unsuspend";
    $action_color = "success";
    $icon = "fa-user-check";
    $confirmation_message = "Are you sure you want to unsuspend this student?";
    $success_redirect = "Student account has been reactivated.";
} else {
    $page_title = "Suspend Student";
    $action_text = "Suspend";
    $action_color = "warning";
    $icon = "fa-user-slash";
    $confirmation_message = "Are you sure you want to suspend this student?";
    $success_redirect = "Student account has been suspended.";
}
?>
...
<head>
    ...
    <!-- Custom CSS for suspend student -->
    <style>
        .action-header {
                    ...
                    <div class="row align-items-center">
                        <div class="col-md-8 text-center text-md-start">
                            ...
                            <p class="text-white-50 mb-0">
                                You are about to <?php echo strtolower($action_text); ?> this student's account.
                            </p>
                        </div>
                        <div class="col-md-4 text-center text-md-end">
                            <div class="text-white-50">
                                ...
                                <small>This action will be recorded in the system logs.</small>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="card student-info-card">
                                <div class="card-body text-center">
                                    <div class="mb-4">
                                        ...
                                        <h4>Student Has Been <?php echo $action_text; ?>ed</h4>
                                        <p class="text-muted"><?php echo $success_redirect; ?></p>
                                    </div>
                                    <div class="d-flex justify-content-center gap-3">
                                        <a href="view_student.php?id=<?php echo $student['id']; ?>" class="btn btn-primary btn-action">
                                            <i class="fas fa-user me-2"></i> View Profile
                                        </a>
                                        <a href="students.php" class="btn btn-outline-secondary btn-action">
                                            <i class="fas fa-users me-2"></i> Back to Students
                                        </a>
                                        <?php if($action === 'suspend'): ?>
                                            <a href="suspend_student.php?id=<?php echo $student['id']; ?>&action=unsuspend" class="btn btn-success btn-action">
                                ...
                                <div class="card-body">
                                    <div class="row align-items-center">
                                        <div class="col-md-2 text-center">
                                            <img src="https://ui-avatars.com/api/?name=<?php echo urlencode($student['name']); ?>&background=<?php echo $action === 'suspend' ? 'e74c3c' : '27ae60'; ?>&color=fff&size=100" 
                                        ...
                                        <div class="col-md-10">
                                            <div class="row">
                                                ...
                                                <div class="col-md-4">
                                                    ...
                                                    <p class="mb-1"><strong>#STU<?php echo str_pad($student['id'], 3, '0', STR_PAD_LEFT); ?></strong></p>
                                                    
                                                    <small class="text-muted">Registration Date</small>
                                                    <p class="mb-0"><?php echo date('M j, Y', strtotime($student['created_at'])); ?></p>
                                                </div>
                                                <div class="col-md-4">
                                                    ...
                                                    <p class="mb-0"><?php echo date('M j, Y', strtotime($student['updated_at'])); ?></p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="card-body">
                                    ...
                                    <form method="POST" id="suspendForm">
                                        <div class="text-center mb-4">
                                            ...
                                            <p class="text-muted">
                                                This action will <?php echo $action === 'suspend' ? 'restrict' : 'restore'; ?> the student's access to the system.
                                            </p>
                                        </div>
                                        ...
                                        <div class="reason-box">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <textarea class="form-control" id="reason" name="reason" rows="3" 
                                                          placeholder="Please provide a reason for suspending this student..." required></textarea>
                                                <div class="form-text">This reason will be recorded and may be reviewed later.</div>
                                            </div>
                                            ...
                                            <ul class="consequences-list">
                                                <?php if($action === 'suspend'): ?>
                                                    <li>
                                                        ...
                                                        Student will not be able to log into their account
                                                    </li>
                                                    <li>
                                                        <i class="fas fa-lock"></i>
                                                        All account features will be temporarily disabled
                                                    </li>
                                                    <li>
                                                        <i class="fas fa-bell"></i>
                                                        Student will receive a notification about the suspension
                                                    </li>
                                                    <li>
                                                ...
                                                <?php else: ?>
                                                    ...
                                                    <li>
                                                        <i class="fas fa-bell"></i>
                                                        Student will be notified about account reactivation
                                                    </li>
                                                    <li>
                                                        <i class="fas fa-history"></i>
                                                        This action will be recorded in the system audit log
                                            ...
                                            <div class="form-check mb-3">
                                                ...
                                                <label class="form-check-label" for="confirmAction">
                                                    <strong>
                                                        ...
                                                        <?php echo strtolower($action_text); ?> this student's account.
                                                    </strong>
                                                </label>
                                            </div>
                                        </div>
        ...
        $(document).ready(function() {
            ...
            $('input[name="duration"]').on('change', function() {
                if ($(this).val() === 'permanent') {
                    if (!confirm('Are you sure you want to set permanent suspension? This action should only be used in severe cases.')) {
